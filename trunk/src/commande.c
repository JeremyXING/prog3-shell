...
int  cd(char ** arguments){
  ...
  fwrite(var_pwd, strlen(var_pwd) * sizeof(char), 1, fichier_tmp);  //puis on re Ècrit la ligne "PWD=" dans ".profile"
...
int echo(char ** arguments){
  ...
  if(LongueurListe(arguments) == 1)
    ...
    for(i = 1; i < LongueurListe(arguments); i++)//pour chaque arguments
      ...
      case '$':  //cas pour lequel la chaine est prÈcÈdÈe du caractËre "$"
	retour = lire_contenu_variable(arguments[i]+1);
	break;
      case '\"': //cas pour lequel la chaine est entourÈe par des doubles quotes
	...
	free(buf);
	break;
      case '\'': //cas pour lequel la chaine est entourÈe par des simples quotes
	buf = malloc((strlen(arguments[i])-2) * sizeof(char));
...
int alias_(char ** arguments){
  ...
  int pos;
  if(argc == 1)
    if((pos = alias_rechercherAlias(arguments[0])) != -1){ //Si l'alias existe, on exÈcute sa commande attachÈe 
